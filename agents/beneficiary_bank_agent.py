"""
Beneficiary Bank AI Agent - Updates credit/CBS integration logic per change.
"""
...
class BeneficiaryBankAgent(BaseAgent):
    """Beneficiary Bank agent that processes manifests and updates credit logic."""
    
    def __init__(self, llm_instance: Optional[LLM] = None):
        """Initialize Beneficiary Bank agent."""
        super().__init__(
            agent_id="BENEFICIARY_BANK_AGENT",
            agent_name="Beneficiary Bank Agent",
...
"""
    ...
    def _generate_basic_changes(self, manifest: ChangeManifest) -> List[Dict[str, Any]]:
        ...
        if manifest.change_type.value == "validation_rule":
            # Add minimum amount validation in _parse_reqpay_credit function.
            ...
            changes.append({
                "file_path": "bene_bank/app.py",  # Relative to base_path
                "changes": {
                    "type": "add_validation",
                    "validation_code": validation_code,
                    "insert_point": "        amount = float(amt.get(\"value\") or 0) if amt is not None else 0.0",
                },
            })
    ...
    def get_component_paths(self) -> List[str]:
        """Get Beneficiary Bank component file paths."""
        return [
            "bene_bank/app.py",
            "bene_bank/db/db.py",
        ]
